...
function checkOldPasssword(oldPassword) {
    "use strict";
    var copy = {};
    var defer = $.Deferred();
    $.extend(copy, userObj, {
    ...
    $.ajax({
        type: "post",
        url: "UserServlet?method=checkPasssword",
        data: copy,
...
(function($) {
    ...
    function require(f) {
        ...
        this.rules = {
            isOldPassword: function(a, b, c, d, e) {
                b = b || " ";
                checkOldPasssword(a).then(function(data) {
                    if (!data)
                        return b;
                });
            },
...
(function($) {
    ...
    $("#aui-btn-reg").click(function() {
        sendMessages["name"] = sendMessages["name"];
        sendMessages["isChecked"] = String($("#agree").prop("checked"));
        if (validNum < 5)
            return false;
        if (sendMessages.length < 7)
            return false;
...
window.addEventListener("load", function() {
    ...
    document.getElementById("nameSubmit").addEventListener("click", function() {
        if (sendMessages["name"]) {
            var copy = {};
            $.extend(copy, userObj, {
                "name": sendMessages["name"],
                "update": "name"
            });
            $.post("UserServlet?method=updateInfo", copy, function(data) {
                var JSONData = JSON.parse(data);
                if (!!JSONData.error)
                    alert(JSONData.error);
                else if (!!JSONData.success)
    ...
    document.getElementById("emailSubmit").addEventListener("click", function() {
        if (sendMessages["email"] && $("#verification").val().length === 6) {
            var copy = {};
            $.extend(copy, userObj, {
                "email": sendMessages["email"],
                "authCode": $("#verification").val(),
                "update": "email"
            });
            $.post("UserServlet?method=updateInfo", copy, function(data) {
                var JSONData = JSON.parse(data);
    ...
    document.getElementById("passwordSubmit").addEventListener("click", function() {
        if (sendMessages["oldPassword"] && sendMessages["password"] && sendMessages["rePassword"]) {
            var copy = {};
            $.extend(copy, userObj, {
                "oldPassword": sendMessages["oldPassword"],
                "password": sendMessages["password"],
                "rePassword":sendMessages["rePassword"],
                "update": "password"
            });
            $.post("UserServlet?method=updateInfo", copy, function(data) {
                var JSONData = JSON.parse(data);
    ...
    document.getElementById("portraitSubmit").addEventListener("click", function() {
        var copy = {};
        $.extend(copy, userObj, {
            "portrait": $("#avatar").eq(0).attr("src"),
            "update": "portrait"
        });
        $.post("UserServlet?method=updateInfo", copy, function(data) {
            var JSONData = JSON.parse(data);
